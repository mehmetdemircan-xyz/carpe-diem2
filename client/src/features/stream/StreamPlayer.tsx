...
import { isMixedContent } from './urlKind';

export type StreamFailure =
  | 'unreachable'
  | 'blocked'
  | 'unsupported'
  /** The browser fetched it and refused to decode it — wrong container. */
  | 'undecodable'
  /** An http source on an https page; the browser blocks it before we try. */
  | 'mixed-content';
...
/**
 * The HLS form of an address that turned out to be a raw transport stream.
 *
 * Live sources very often publish the same channel twice: once as an endless
 * MPEG-TS body, which no browser can decode, and once as an `.m3u8` playlist,
 * which every browser can. The two differ only by the extension, so when the
 * first one fails there is one obvious thing left to try before giving up.
 *
 * This is a retry of the address the person already supplied, on the same
 * host, with whatever credentials that address already carried. It reads
 * nothing, discovers nothing, and works only where the source publishes the
 * playlist openly — if it does not, the attempt simply fails like any other
 * bad address, and the failure it reports is the true one.
 */
function hlsSibling(url: string): string | null {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (/\.(ts|mts|m2ts|tsv)$/i.test(path)) {
    parsed.pathname = path.replace(/\.[^./]+$/, '.m3u8');
    return parsed.toString();
  }

  // An address whose last segment carries no extension at all — the shape
  // live endpoints usually take. Anything with some other extension is a
  // deliberate format and is left alone.
  const lastSegment = path.slice(path.lastIndexOf('/') + 1);
  if (lastSegment.length > 0 && !lastSegment.includes('.')) {
    parsed.pathname = `${path}.m3u8`;
    return parsed.toString();
  }
...
export const StreamPlayer = memo(function StreamPlayer({
  ...
  /**
   * The one retry, tagged with the address it belongs to. Tagging is what
   * makes a new stream drop it during render rather than in an effect, so a
   * changed URL is never briefly played through the previous one's fallback.
   */
  const [fallback, setFallback] = useState<{ from: string; to: string } | null>(null);
  const sourceUrl = fallback?.from === stream.url ? fallback.to : stream.url;
  const usingFallback = sourceUrl !== stream.url;
  ...
  useEffect(() => {
    ...
    const attach = async () => {
      // Checked first: the browser refuses this before any request is made,
      // so every downstream error would be a symptom rather than the cause.
      if (isMixedContent(sourceUrl)) {
        onErrorRef.current('mixed-content');
        return;
      }

      if (!looksLikeHls(sourceUrl) || hasNativeHls(video)) {
      ...
      /**
       * Ask hls.js whether this is a live edge rather than inferring it from
       * the element's duration. hls.js leaves `liveDurationInfinity` off by
       * default, so a live stream reports a finite duration through MSE and a
       * duration check would classify every broadcast as a recording — which
       * would light the wrong badge and, worse, let followers seek on a
       * stream that has nothing to seek to.
       */
      hls.on(HlsModule.Events.LEVEL_LOADED, (_event, data) => {
        if (!disposed) setIsLive(Boolean(data.details?.live));
      });
  ...
  useEffect(() => {
    ...
    const onLoaded = () => {
      setReady(true);
      // Only meaningful on the native-HLS path (Safari), where a live stream
      // genuinely reports an infinite duration. The MSE path is told by
      // hls.js instead, and must not be overruled here.
      if (!looksLikeHls(sourceUrl) || hasNativeHls(video)) {
    ...
    const onNativeError = () => {
      ...
      // Both of these mean the bytes arrived and the browser would not turn
      // them into pictures: no decoder for the container, or a decode that
      // failed outright. A raw MPEG-TS endpoint is the usual cause, and
      // "check the link" would send someone hunting for a problem that is
      // not there.
      const code = video.error?.code;
      const undecodable =
        code === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED || code === MediaError.MEDIA_ERR_DECODE;

      // One retry at the playlist form of the same address before reporting
      // anything. If that fails too, its own error is the one that surfaces,
      // which is how a CORS refusal ends up being named as a CORS refusal
      // instead of hiding behind "no decoder".
      if (undecodable && !usingFallback) {
        const sibling = hlsSibling(stream.url);
        if (sibling) {
          console.warn('[carpe] retrying as hls', sibling);
          setFallback({ from: stream.url, to: sibling });
          return;
        }
      }

      onErrorRef.current(undecodable ? 'undecodable' : 'unreachable');
    };
  ...
  }, [stream.url, sourceUrl, usingFallback]);
  ...
  useEffect(() => {
    ...
    // A live edge has no shared position to agree on, so only the play/pause
    // state is worth broadcasting there.
    const report = () =>
      onPlaybackChangeRef.current(!video.paused, isLive ? 0 : video.currentTime);
      ...
      {!ready && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <p className="rounded-md bg-black/60 px-3 py-1.5 text-xs text-chalk-200">
            {t(usingFallback ? 'stream.tryingHls' : 'stream.loading')}
          </p>
        </div>
      )}
