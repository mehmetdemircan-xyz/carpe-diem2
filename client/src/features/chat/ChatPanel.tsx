...
export const ChatPanel = memo(function ChatPanel({
  ...
  return (
    /*
      Below the video on a phone held upright, beside it on a phone held
      sideways. Sideways there is barely any height to divide, so a panel
      under the picture would leave the picture postage-stamp sized — exactly
      what turning the phone was meant to fix.

      The entrance is a fade rather than a slide, deliberately. A translate
      means the panel's resting position depends on an animation reaching its
      end: anything that restarts or stalls it leaves the panel parked a full
      panel-height away, taking the message field off the bottom of the
      screen with it. That is a layout bug wearing a decoration's clothes,
      and it was a real one here. Opacity cannot displace anything.
    */
    <aside
      className="flex h-[38%] min-h-[8.5rem] max-h-[26rem] w-full shrink-0 animate-fade-in flex-col
                 border-t border-ink-800 bg-ink-900
                 squat:h-auto squat:min-h-0 squat:max-h-none squat:w-64
                 squat:border-l squat:border-t-0
                 lg:h-auto lg:min-h-0 lg:max-h-none lg:w-80
