...
/**
 * Whether the page may touch capture devices at all.
 *
 * `navigator.mediaDevices` is simply absent outside a secure context, so an
 * app served over plain http to anything other than localhost has no
 * microphone and no screen capture — and, because the object is missing
 * rather than throwing, every feature that needs it fails looking like a bug
 * in the app. A phone opening `http://192.168.x.x` is the common way to meet
 * this, since localhost's exemption does not travel to another device.
 */
...
export async function captureMicrophone(
  ...
  { ok: true; stream: MediaStream } | { ok: false; error: 'denied' | 'unavailable' | 'insecure' }
> {
  // Checked before the capability test, because an insecure context is why
  // the capability is missing and is the only one of the two anybody can fix.
  if (!isSecureMediaContext()) return { ok: false, error: 'insecure' };
