...
export function RoomPage({
  ...
  // Say why capture is unavailable, once, on arrival. An insecure context is
  // checked first and outranks the mobile message: over plain http nothing
  // gets a microphone either, and blaming the phone would send someone
  // looking for a browser limitation instead of the http:// in their address
  // bar — which is the half of it they can actually fix.
  useEffect(() => {
    if (!isSecureMediaContext()) {
      toast.push(t('room.insecureContext'), 'error');
    } else if (!shareSupported && isLikelyMobile()) {
  ...
  const onStreamError = useCallback(
    (failure: StreamFailure) => {
      const MESSAGE = {
        blocked: 'error.streamBlocked',
        unsupported: 'error.streamUnsupported',
        undecodable: 'error.streamUndecodable',
        'mixed-content': 'error.streamMixedContent',
        unreachable: 'error.streamUnreachable',
      } as const;

      toast.push(t(MESSAGE[failure]), 'error');
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Sideways on a phone the header is the cheapest thing to shrink: it
          holds the room code and a status dot, neither of which is worth a
          centimetre of picture. */}
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-ink-800 bg-ink-900 px-3 py-2.5 squat:py-1 sm:px-4">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        */}
        {/* min-h-0 is load-bearing: without it this column's min-height is
            auto, so it refuses to shrink below the stage plus the chat's own
            minimum and the chat slides out under the control bar the moment
            the viewport gets short — which is exactly what an open keyboard
            does. */}
        <div className="flex min-h-0 min-w-0 flex-1 flex-col squat:flex-row lg:flex-row">
