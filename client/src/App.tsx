...
export function App() {
  ...
  // Sizes the app to what is visible rather than to the layout viewport, so
  // an open keyboard shrinks the app instead of scrolling the video away.
  useViewportHeight();
  ...
  const ptt = usePushToTalk(audio.pushToTalk, audio.pushToTalkKey);
  const session = useRoomSession(quality, audio, ptt.talking);
  ...
  if (screen.name === 'room') {
    return (
      <RoomPage
        ...
        talking={ptt.talking}
        onTalkPress={ptt.press}
        onTalkRelease={ptt.release}
        onLeave={goHome}
      />
    );
  }
