...
export function registerHandlers(io: IO, socket: AppSocket, store: RoomStore): void {
  ...
  socket.on('stream:load-playlist', (payload, rawAck) => {
    ...
    void loadPlaylist(payload?.url).then((result) => {
      if (!result.ok) {
        // The status travels with the code so the person pasting the link can
        // tell "the host rejected me" from "the host never answered" without
        // reading a server log they do not have.
        switch (result.reason) {
          case 'invalid-url':
            return ack(fail('INVALID_URL'));
          case 'refused':
            return ack({ ok: false, error: 'PLAYLIST_REFUSED', detail: String(result.status) });
          case 'timeout':
            return ack(fail('PLAYLIST_TIMEOUT'));
          case 'too-large':
            return ack({
              ok: false,
              error: 'PLAYLIST_TOO_LARGE',
              // Naming the ceiling turns "it failed" into "raise this by
              // this much", which is the only actionable form of this one.
              detail: `${Math.round(config.maxPlaylistBytes / (1024 * 1024))} MB`,
            });
          case 'not-a-list':
            return ack(fail('PLAYLIST_NOT_A_LIST'));
          case 'empty':
            return ack(fail('PLAYLIST_EMPTY'));
          default:
            return ack(fail('PLAYLIST_UNREACHABLE'));
        }
      }
