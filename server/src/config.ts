...
export const config = {
  ...
  /**
   * How long the playlist fetcher waits for an upstream host. Large channel
   * lists over a slow link legitimately take longer than a page load, so this
   * is generous; lower it if your users only ever paste small playlists.
   */
  playlistFetchTimeoutMs: int(process.env.PLAYLIST_FETCH_TIMEOUT_MS, 20_000),

  /**
   * Ceiling on a playlist body, in megabytes. Big provider lists are genuinely
   * large, so this is generous — but it cannot be removed altogether: the body
   * is read into memory to be parsed, and without a ceiling a host that never
   * stops sending would take the server down with it. Raise it if your lists
   * are bigger; the failure names the limit, so you will know.
   */
  maxPlaylistBytes: int(process.env.MAX_PLAYLIST_MB, 64) * 1024 * 1024,
