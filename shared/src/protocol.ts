...
/**
 * Provider channel lists run to tens of thousands of entries, and a channel
 * you cannot see is a channel you cannot pick — the picker's own search is
 * what makes a list this size usable, and it can only search what it was
 * given. The cap is a backstop against an absurd response, not a curation
 * decision, so it sits above any real list.
 */
export const MAX_PLAYLIST_ENTRIES = 20_000;
...
/**
 * `detail` carries a short, non-sensitive fact that makes a failure
 * actionable — an HTTP status from an upstream host, a size limit that was
 * exceeded. It is never the message shown to the person: the client picks
 * that from its own catalogue by `error`, and appends the detail if present.
 * Server internals never go in here.
 */
export type Ack<T> =
  | { ok: true; data: T }
