...
import { classifyUrl, displayHost, type UrlKind } from './urlKind';
...
interface StreamModalProps {
  ...
  onLoadPlaylist: (
    ...
    { ok: true; entries: PlaylistEntry[]; truncated: boolean } | { ok: false; error: ErrorCode }
  >;
...
export const StreamModal = memo(function StreamModal({
  ...
  const submit = useCallback(
    ...
    [url, busy, classified.kind, mode, onLoadPlaylist, onStart, close],
  );
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="field text-sm"
          />
          <UrlVerdict mode={mode} kind={url.trim() ? classified.kind : null} />
        </div>
 ...
 */
function UrlVerdict({ mode, kind }: { mode: StreamModalMode; kind: UrlKind | null }) {
  const t = useT();
