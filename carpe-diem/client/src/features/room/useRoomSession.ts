...
export function useRoomSession(
    ...
    (on: boolean) => {
      ...
      void (async () => {
        ...
        if (!result.ok) {
          ...
          toastRef.current.push(
            tRef.current(result.error === 'denied' ? 'toast.micDenied' : 'toast.micUnavailable'),
            'error',
          );
          return;
        }
  ...
  const loadPlaylist = useCallback(async (url: string) => {
    ...
    const ack = await request<{ entries: PlaylistEntry[]; truncated: boolean }>(
      ...
      { url },
      20_000, // The server has to fetch and parse it, which can be slow.
    );
    if (!ack.ok) return { ok: false as const, error: ack.error };
